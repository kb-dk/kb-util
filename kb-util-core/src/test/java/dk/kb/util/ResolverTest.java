...
class ResolverTest {
    ...
    void resolveGlob() throws IOException {
        URL url1 = Resolver.resolveURL("resolver/testfile.txt");
        ...
        assertThat("Sanity check path should work", Files.exists(Path.of(parent2, "resolver/testfile.txt")));

        assertThat("The right number of files should be resolved for file wildcard",
                   Resolver.resolveGlob(parent2 + "/resolver/globfile*.txt").size() == 2);
        ...
        assertThat("The right number of files should be resolved for path wildcard",
                   Resolver.resolveGlob(parent2 + "/*/globfile*.txt").size() == 2);

        assertThat("The right number of files should be resolved for bracket wildcard",
                   Resolver.resolveGlob(parent2 + "/resolver/globfile[2-3].txt").size() == 1);

        assertThat("The right number of files should be resolved combining globs",
                   Resolver.resolveGlob(parent2 + "/re[slm]olver/glob*[1-2].tx?").size() == 2);
    }

    @Test
    void resolveDottedGlob() throws IOException {
        URL url1 = Resolver.resolveURL("resolver/subfolder/somefile.txt");
        ...
        assertThat("The file '" + known + "' should be resolved with /./ in the path",
                   is(Resolver.resolveGlob("resolver/subfolder/./somefile.txt").size()).matches(1));

        assertThat("The file '" + known + "' should be resolved with /../ in the path",
                   is(Resolver.resolveGlob("resolver/subfolder/../subfolder/somefile.txt").size()).matches(1));

        String glob = "../" + parent + "/resolver/subfolder/somefile.txt";
    ...
    void resolveGlobRelative() {
        assertThat("Globbing relative to the class path should work",
                   Resolver.resolveGlob("resolver/globfile*.txt").size() == 2);
    }
